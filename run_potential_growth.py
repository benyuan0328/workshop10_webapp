...
def analyze_potential_growth_data(data: str,
    ...
    # '1_全部系列「潛在」全年成長比較分析表v2.csv'
    # '2_全部系列「潛在」每月成長比較分析表.csv'
    series_data_month_growth_all_year_df, series_data_month_growth_all_year_df_top, series_data_month_growth_df = potiential_growth_all(
            data = data,
            column_name = column_name,
            rank = 0,
    ...
    fig = visualize_and_handle_files(series_data_month_growth_all_year_df_top, 
                               column_name, 
                               compare_year1, compare_year2,
                               which_series)

    return series_data_month_growth_all_year_df, series_data_month_growth_all_year_df_top, series_data_month_growth_df, fig
...
def run_potential_growth():
    ...
    # 設定 要選擇的月份
    month_end = st.sidebar.slider('請選擇要分析的月份', 1, 12, 9)
    
    # 設定 成長率的基礎月份
    size = st.sidebar.number_input('成長率的比較至少要幾個月份', 1, 12, 5)
    ...
    if st.button('開始分析', disabled=st.session_state.running, key='run_button'):
        ...
        series_data_month_growth_all_year_df, series_data_month_growth_all_year_df_top, series_data_month_growth_df, fig = analyze_potential_growth_data(
            data = sales_data ,
            column_name = product,
            compare_year1_str = str(year_start),
            compare_year2_str = str(year_end),
            month_end = month_end,
            product_profit_selected = product_profit_selected,
            size = size
        )
        ...
        # Display
        st.subheader('總體「潛在」成長比較分析')
        
        st.markdown('#### 全部系列「潛在」全年成長比較分析表')
        st.dataframe(series_data_month_growth_all_year_df)
        generate_download_button(df=series_data_month_growth_all_year_df, filename="總體「潛在」成長比較分析")
        
        st.markdown('#### 全部系列「潛在」全年成長比較分析圖')
        st.plotly_chart(fig)
        
        st.write('---')
        
        st.subheader('全部系列「潛在」全年成長比較分析表')
        st.dataframe(series_data_month_growth_all_year_df_top)
        generate_download_button(df=series_data_month_growth_all_year_df_top, filename="全部系列「潛在」全年成長比較分析表")
        

        st.subheader('全部系列「潛在」每月成長比較分析表')
        series_data_month_growth_df
        st.dataframe(series_data_month_growth_df)
        generate_download_button(df=series_data_month_growth_df, filename="全部系列「潛在」每月成長比較分析表")
